...
@section('content')
    <div class="container my-4 pb-5" style="max-width:480px">
        ...
        {{-- FORM checkout --}}
        <form id="cartForm" action="{{ route('cafe.checkout.store') }}" method="POST">
            @csrf
            ...
            {{-- Hidden input untuk bawa cart --}}
            <input type="hidden" name="cart" id="cartInput">
            <!-- Label -->
            <label class="text-muted d-block mb-1">Meja</label>

            <!-- Tampilan meja -->
            <div class="p-3 rounded bg-light border d-flex align-items-center">
                <i class="bi bi-table me-2 text-brown"></i>
                <span class="fw-bold text-dark">MEJA-01</span>
            </div>

            <!-- Hidden input untuk dikirim ke backend -->
            <input type="hidden" name="meja" value="MEJA-01">
        </form>
    </div>
    ...
    <div class="fixed-bottom bg-white border-top">
        <div class="container py-3 d-flex gap-2 justify-content-center" style="max-width:480px">
            <button id="btnClearCart" class="btn btn-outline-secondary w-25" type="button">Kosongkan</button>
            <button id="btnCheckout" class="btn btn-brown flex-grow-1 rounded-pill fw-semibold" disabled type="submit"
                form="cartForm">
                Lanjut ke Checkout
            </button>
        </div>
      ...
      <div class="d-flex align-items-start gap-3">
        ...
        <div class="flex-grow-1">
          <div class="d-flex justify-content-between align-items-start">
            ...
            <button type="button" class="btn btn-link text-danger p-0 small" data-action="remove" data-id="${id}">Hapus</button>
          </div>

          <div class="d-flex justify-content-between align-items-center mt-2">
            <div class="btn-group btn-group-sm" role="group" aria-label="Kuantitas">
              <button type="button" class="btn btn-outline-brown" data-action="minus" data-id="${id}" aria-label="Kurangi">-</button>
              <button type="button" class="btn btn-light" disabled style="min-width:2.25rem" data-role="qty">${qty}</button>
              <button type="button" class="btn btn-brown" data-action="plus" data-id="${id}" aria-label="Tambah">+</button>
            </div>
